...
class Parser:
    ...
    def load_models(self, loc=None):
        ...
        self.word2Idx = np.load(
            os.path.join(loc, "word2Idx.npy"), allow_pickle=True
        ).item()
        # loading idx2Label
        self.idx2Label = np.load(
            os.path.join(loc, "idx2Label.npy"), allow_pickle=True
        ).item()
    ...
    def getCasing(self, word, caseLookup):
        ...
        if word.isdigit():  # Is a digit
            ...
            0
        ].isupper():  # is a title, initial char upper, then all lower
            casing = "initialUpper"
        elif numDigits > 0:
    ...
    def predict(self, Sentence):
        ...
        Sentence = self.padding(
            self.createTensor(
                Sentence, self.word2Idx, self.case2Idx, self.char2Idx
            )
        )
